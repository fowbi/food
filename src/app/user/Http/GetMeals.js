...
const action = async (request, response, next) => {
  const when = moment(request.params.when).utc();
  ...
  await models.meal.findAll({ where: { user: user.id, when: { [operation.gte]: startDate.format(), [operation.lte]: endDate.format() } } })
    .then(function(meals) {
